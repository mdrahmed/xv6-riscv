...
#include "file.h"
#include "fcntl.h"
#include "kernel/new_syscall.h"


uint64 create_the_buffer(char nameBuf[16], int open_or_close, uint64 *address_64bit_ring_buffer_mapped);
////new sys ringbuf
//uint64 sys_ringbuf(uint64 address_of_string, int open_or_close, uint64 address_64bit_ring_buffer_mapped)
uint64 sys_ringbuf(void)
{
  ...
  char namestr[16];
  int open_or_close;
  uint64 *address_64bit_ring_buffer_mapped;
  ...
  // struct check c;
  // c.ss = namestr;
  // uint64 first_va = create_the_buffer(namestr, open_or_close);
