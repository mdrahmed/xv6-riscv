...
int create_or_close_the_buffer_user(char name[16], int open_close, uint64 **addr){
  ...
  ac:	9f848493          	addi	s1,s1,-1544 # aa0 <rings+0x8>
  b0:	00001917          	auipc	s2,0x1
  b4:	ae090913          	addi	s2,s2,-1312 # b90 <base+0x8>
    }
  }
  else{
    for(i = 0; i < 10; i++){
  ...
  bc:	9d0b0b13          	addi	s6,s6,-1584 # a88 <vm_addr>
  if(open_close == 1){
  c0:	04f59063          	bne	a1,a5,100 <create_or_close_the_buffer_user+0x74>
      if(rings[i].exists == 0){
  c4:	00001497          	auipc	s1,0x1
  c8:	9e44a483          	lw	s1,-1564(s1) # aa8 <rings+0x10>
  cc:	c099                	beqz	s1,d2 <create_or_close_the_buffer_user+0x46>
  ce:	4481                	li	s1,0
  d0:	a899                	j	126 <create_or_close_the_buffer_user+0x9a>
        ringbuf(name, open_close, &vm_addr);
  d2:	865a                	mv	a2,s6
  d4:	4585                	li	a1,1
  d6:	00000097          	auipc	ra,0x0
  da:	4da080e7          	jalr	1242(ra) # 5b0 <ringbuf>
        rings[i].book->write_done = 0;
  de:	00001797          	auipc	a5,0x1
  e2:	9ba78793          	addi	a5,a5,-1606 # a98 <rings>
  e6:	6798                	ld	a4,8(a5)
  e8:	00073423          	sd	zero,8(a4)
        rings[i].book->read_done = 0;
  ec:	6798                	ld	a4,8(a5)
  ee:	00073023          	sd	zero,0(a4)
        rings[i].exists++;
  f2:	4b98                	lw	a4,16(a5)
  f4:	2705                	addiw	a4,a4,1
  f6:	cb98                	sw	a4,16(a5)
        break;
  f8:	a03d                	j	126 <create_or_close_the_buffer_user+0x9a>
    for(i = 0; i < 10; i++){
  fa:	04e1                	addi	s1,s1,24
  fc:	03248463          	beq	s1,s2,124 <create_or_close_the_buffer_user+0x98>
      if(rings[i].exists != 0){
 100:	449c                	lw	a5,8(s1)
 102:	dfe5                	beqz	a5,fa <create_or_close_the_buffer_user+0x6e>
        ringbuf(name, open_close, &vm_addr);
 104:	865a                	mv	a2,s6
 106:	85ce                	mv	a1,s3
 108:	8552                	mv	a0,s4
 10a:	00000097          	auipc	ra,0x0
 10e:	4a6080e7          	jalr	1190(ra) # 5b0 <ringbuf>
        rings[i].book->write_done = 0;
 112:	609c                	ld	a5,0(s1)
 114:	0007b423          	sd	zero,8(a5)
        rings[i].book->read_done = 0;
 118:	609c                	ld	a5,0(s1)
 11a:	0007b023          	sd	zero,0(a5)
        rings[i].exists = 0;
 11e:	0004a423          	sw	zero,8(s1)
 122:	bfe1                	j	fa <create_or_close_the_buffer_user+0x6e>
    for(i = 0; i < 10; i++){
 124:	44a9                	li	s1,10
 ...
 12a:	9627b783          	ld	a5,-1694(a5) # a88 <vm_addr>
 12e:	00fab023          	sd	a5,0(s5)
  return i;
}
 132:	8526                	mv	a0,s1
...
0000000000000148 <ringbuf_start_write>:
...
void ringbuf_start_write(int ring_desc, uint64 **addr, int *bytes){ 
 ...
 158:	9347b783          	ld	a5,-1740(a5) # a88 <vm_addr>
 15c:	e19c                	sd	a5,0(a1)
  if(*bytes == 0){
 15e:	421c                	lw	a5,0(a2)
 160:	e79d                	bnez	a5,18e <ringbuf_start_write+0x46>
    *bytes = (4096*16) -(rings[ring_desc].book->write_done - rings[0].book->read_done);
 162:	00001697          	auipc	a3,0x1
 166:	93668693          	addi	a3,a3,-1738 # a98 <rings>
 16a:	669c                	ld	a5,8(a3)
 16c:	6398                	ld	a4,0(a5)
 ...
 1ae:	bfd9                	j	184 <ringbuf_start_write+0x3c>
