...
go(int which_child)
{
  ...
  int fd = -1;
     ...
     100:	7e4a0a13          	addi	s4,s4,2020 # 18e0 <buf.0>
...
iter()
{
  ...
  if(pid2 == 0){
    ...
    rand_next = 7177;
     b3e:	6789                	lui	a5,0x2
     b40:	c0978793          	addi	a5,a5,-1015 # 1c09 <buf.0+0x329>
...
int create_or_close_the_buffer_user(char name[16], int open_close){
  ...
  if(open_close == 1){
     ...
     c36:	0a648493          	addi	s1,s1,166 # 1cd8 <rings+0x10>
     c3a:	00001917          	auipc	s2,0x1
     c3e:	18e90913          	addi	s2,s2,398 # 1dc8 <__BSS_END__>
     c42:	04f59563          	bne	a1,a5,c8c <create_or_close_the_buffer_user+0x70>
    for(i = 0; i < 10; i++){
      if(rings[i].exists == 0){
     c46:	00001497          	auipc	s1,0x1
     c4a:	0924a483          	lw	s1,146(s1) # 1cd8 <rings+0x10>
     ...
     c58:	07490913          	addi	s2,s2,116 # 1cc8 <rings>
     c5c:	00093603          	ld	a2,0(s2)
     c60:	4585                	li	a1,1
     c62:	00000097          	auipc	ra,0x0
     c66:	4de080e7          	jalr	1246(ra) # 1140 <ringbuf>
    ...
    for(i = 0; i < 10; i++){
     ...
     c9c:	4a8080e7          	jalr	1192(ra) # 1140 <ringbuf>
        rings[i].exists = 0;
...
void ringbuf_start_write(int ring_desc, uint64 *addr, int *bytes){ // address ta double pointer hobe
  ...
  *addr = (4096*16)-(rings[ring_desc].book->write_done++) % 4096;
     ...
     cd2:	ffa70713          	addi	a4,a4,-6 # 1cc8 <rings>
     cd6:	97ba                	add	a5,a5,a4
     cd8:	6798                	ld	a4,8(a5)
     cda:	671c                	ld	a5,8(a4)
     cdc:	00178693          	addi	a3,a5,1
     ce0:	e714                	sd	a3,8(a4)
     ce2:	17d2                	slli	a5,a5,0x34
     ce4:	93d1                	srli	a5,a5,0x34
     ce6:	6741                	lui	a4,0x10
     ce8:	40f707b3          	sub	a5,a4,a5
     cec:	e19c                	sd	a5,0(a1)
  if(*bytes == 0){
     cee:	421c                	lw	a5,0(a2)
     cf0:	e79d                	bnez	a5,d1e <ringbuf_start_write+0x64>
    *bytes = 4096*16 -(rings[ring_desc].book->write_done - rings[0].book->read_done);
     cf2:	00001697          	auipc	a3,0x1
     cf6:	fd668693          	addi	a3,a3,-42 # 1cc8 <rings>
     cfa:	669c                	ld	a5,8(a3)
     cfc:	6398                	ld	a4,0(a5)
    ...
    *bytes = load(&(rings[ring_desc].book->write_done));
     ...
     d2a:	fa278793          	addi	a5,a5,-94 # 1cc8 <rings>
     ...
     d3e:	bfd9                	j	d14 <ringbuf_start_write+0x5a>
...
void ringbuf_finish_write(int ring_desc, int bytes){
  ...
  store(&(rings[ring_desc].book->write_done), bytes);
     ...
     d54:	f7850513          	addi	a0,a0,-136 # 1cc8 <rings>
...
void check_bytes_written(int ring_desc, int *bytes){
  ...
  *bytes = load(&(rings[ring_desc].book->write_done));
     ...
     d8a:	f4250513          	addi	a0,a0,-190 # 1cc8 <rings>
...
void ringbuf_start_read(int ring_desc, uint64 *addr, int *bytes){ // address ta double pointer hobe
  ...
  if(rings[ring_desc].book -> read_done == 0){
     ...
     dc0:	f0c50513          	addi	a0,a0,-244 # 1cc8 <rings>
...
void ringbuf_finish_read(int ring_desc, int bytes){
  ...
  store(&(rings[ring_desc].book->read_done), bytes);
     ...
     e16:	eb650513          	addi	a0,a0,-330 # 1cc8 <rings>
...
memmove(void *vdst, const void *vsrc, int n)
{
  ...
  dst = vdst;
    ...
    100e:	fed70fa3          	sb	a3,-1(a4) # ffff <__global_pointer$+0xdf36>
    while(n-- > 0)
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    ...
    1464:	4787b783          	ld	a5,1144(a5) # 18d8 <freep>
    ...
    1492:	00e6ea63          	bltu	a3,a4,14a6 <free+0x50>
{
  ...
  freep = p;
    ...
    14d4:	40f73423          	sd	a5,1032(a4) # 18d8 <freep>
}
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    ...
    1506:	3d653503          	ld	a0,982(a0) # 18d8 <freep>
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    152e:	3ae90913          	addi	s2,s2,942 # 18d8 <freep>
  if(p == (char*)-1)
    1532:	5afd                	li	s5,-1
    1534:	a895                	j	15a8 <malloc+0xca>
    base.s.ptr = freep = prevp = &base;
    1536:	00001797          	auipc	a5,0x1
    153a:	88278793          	addi	a5,a5,-1918 # 1db8 <base>
    153e:	00000717          	auipc	a4,0x0
    1542:	38f73d23          	sd	a5,922(a4) # 18d8 <freep>
    ...
    156a:	36a73923          	sd	a0,882(a4) # 18d8 <freep>
      return (void*)(p + 1);
