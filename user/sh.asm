...
0000000000000a60 <main>:
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
     ...
     a9c:	c0848493          	addi	s1,s1,-1016 # 16a0 <buf.0>
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
     aa0:	06300913          	li	s2,99
     aa4:	02000993          	li	s3,32
      if(chdir(buf+3) < 0)
     aa8:	00001a17          	auipc	s4,0x1
     aac:	bfba0a13          	addi	s4,s4,-1029 # 16a3 <buf.0+0x3>
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
     ...
     b36:	b6e50513          	addi	a0,a0,-1170 # 16a0 <buf.0>
...
int create_or_close_the_buffer_user(char name[16], int open_close){
  ...
  if(open_close == 1){
     ...
     b9a:	b8248493          	addi	s1,s1,-1150 # 1718 <rings+0x10>
     b9e:	00001917          	auipc	s2,0x1
     ba2:	c6a90913          	addi	s2,s2,-918 # 1808 <__BSS_END__>
     ba6:	04f59563          	bne	a1,a5,bf0 <create_or_close_the_buffer_user+0x70>
    for(i = 0; i < 10; i++){
      if(rings[i].exists == 0){
     baa:	00001497          	auipc	s1,0x1
     bae:	b6e4a483          	lw	s1,-1170(s1) # 1718 <rings+0x10>
     ...
     bbc:	b5090913          	addi	s2,s2,-1200 # 1708 <rings>
     bc0:	00093603          	ld	a2,0(s2)
     bc4:	4585                	li	a1,1
     bc6:	00000097          	auipc	ra,0x0
     bca:	4de080e7          	jalr	1246(ra) # 10a4 <ringbuf>
    ...
    for(i = 0; i < 10; i++){
     ...
     c00:	4a8080e7          	jalr	1192(ra) # 10a4 <ringbuf>
        rings[i].exists = 0;
...
void ringbuf_start_write(int ring_desc, uint64 *addr, int *bytes){ // address ta double pointer hobe
  ...
  *addr = (4096*16)-(rings[ring_desc].book->write_done++) % 4096;
     ...
     c36:	ad670713          	addi	a4,a4,-1322 # 1708 <rings>
     c3a:	97ba                	add	a5,a5,a4
     c3c:	6798                	ld	a4,8(a5)
     c3e:	671c                	ld	a5,8(a4)
     c40:	00178693          	addi	a3,a5,1
     c44:	e714                	sd	a3,8(a4)
     c46:	17d2                	slli	a5,a5,0x34
     c48:	93d1                	srli	a5,a5,0x34
     c4a:	6741                	lui	a4,0x10
     c4c:	40f707b3          	sub	a5,a4,a5
     c50:	e19c                	sd	a5,0(a1)
  if(*bytes == 0){
     c52:	421c                	lw	a5,0(a2)
     c54:	e79d                	bnez	a5,c82 <ringbuf_start_write+0x64>
    *bytes = 4096*16 -(rings[ring_desc].book->write_done - rings[0].book->read_done);
     c56:	00001697          	auipc	a3,0x1
     c5a:	ab268693          	addi	a3,a3,-1358 # 1708 <rings>
     c5e:	669c                	ld	a5,8(a3)
     c60:	6398                	ld	a4,0(a5)
    ...
    *bytes = load(&(rings[ring_desc].book->write_done));
     ...
     c8e:	a7e78793          	addi	a5,a5,-1410 # 1708 <rings>
     ...
     ca2:	bfd9                	j	c78 <ringbuf_start_write+0x5a>
...
void ringbuf_finish_write(int ring_desc, int bytes){
  ...
  store(&(rings[ring_desc].book->write_done), bytes);
     ...
     cb8:	a5450513          	addi	a0,a0,-1452 # 1708 <rings>
...
void check_bytes_written(int ring_desc, int *bytes){
  ...
  *bytes = load(&(rings[ring_desc].book->write_done));
     ...
     cee:	a1e50513          	addi	a0,a0,-1506 # 1708 <rings>
...
void ringbuf_start_read(int ring_desc, uint64 *addr, int *bytes){ // address ta double pointer hobe
  ...
  if(rings[ring_desc].book -> read_done == 0){
     ...
     d24:	9e850513          	addi	a0,a0,-1560 # 1708 <rings>
...
void ringbuf_finish_read(int ring_desc, int bytes){
  ...
  store(&(rings[ring_desc].book->read_done), bytes);
     ...
     d7a:	99250513          	addi	a0,a0,-1646 # 1708 <rings>
...
memmove(void *vdst, const void *vsrc, int n)
{
  ...
  dst = vdst;
     ...
     f72:	fed70fa3          	sb	a3,-1(a4) # ffff <__global_pointer$+0xe17e>
    while(n-- > 0)
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    ...
    13c8:	2d47b783          	ld	a5,724(a5) # 1698 <freep>
    ...
    13f6:	00e6ea63          	bltu	a3,a4,140a <free+0x50>
{
  ...
  freep = p;
    ...
    1438:	26f73223          	sd	a5,612(a4) # 1698 <freep>
}
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    ...
    146a:	23253503          	ld	a0,562(a0) # 1698 <freep>
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    1492:	20a90913          	addi	s2,s2,522 # 1698 <freep>
  if(p == (char*)-1)
    1496:	5afd                	li	s5,-1
    1498:	a895                	j	150c <malloc+0xca>
    base.s.ptr = freep = prevp = &base;
    149a:	00000797          	auipc	a5,0x0
    149e:	35e78793          	addi	a5,a5,862 # 17f8 <base>
    14a2:	00000717          	auipc	a4,0x0
    14a6:	1ef73b23          	sd	a5,502(a4) # 1698 <freep>
    ...
    14ce:	1ca73723          	sd	a0,462(a4) # 1698 <freep>
      return (void*)(p + 1);
